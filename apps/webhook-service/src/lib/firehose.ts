...
import { BunFirehose, isBun, type CommitEvt, type Event } from '@wispplace/bun-firehose';
import { Firehose } from '@atproto/sync';
import type { Main as WhRecord } from '@wispplace/lexicons/types/place/wisp/v2/wh';
import { createLogger } from '@wispplace/observability';
import { config } from '../config';
import { getCached, setCached, invalidate } from './registry';
import { upsertWebhookRecord, deleteWebhookRecord, findWebhooksForDid, findBacklinkWebhooks } from './db';
...
async function handleEvent(evt: Event | CommitEvt): Promise<void> {
  ...
  if (collection === 'place.wisp.v2.wh') {
    ...
    if (event === 'delete') {
      deleteWebhookRecord(did, rkey).catch(err =>
        logger.error(`[DB] Failed to delete webhook ${did}/${rkey}`, err)
      );
    } else if (record) {
  ...
  await Promise.allSettled(
    matched.map(entry =>
      deliverWebhook(entry, did, collection, rkey, event, cid?.toString(), record)
    )
  );
  } catch (err) {

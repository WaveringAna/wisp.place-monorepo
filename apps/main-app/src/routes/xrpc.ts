...
const claimWispSubdomain = async (did: DidString, input: { handle: string; siteRkey?: string }) => {
...
const claimCustomDomainForDid = async (did: DidString, input: { domain: string; siteRkey?: string }) => {
	...
	if (existing?.verified && existing.did !== did) {
		alreadyClaimed('domain is already claimed')
	}
...
const mapDomainToSiteForDid = async (did: DidString, input: { domain: string; siteRkey: string }) => {
	const domain = normalizeDomain(input.domain)
	if (domain.length === 0) {
		invalidDomain('domain is required')
	}
...
const deleteSiteForDid = async (did: DidString, input: { siteRkey: string }) => {
	const siteRkey = input.siteRkey.trim()
	if (siteRkey.length === 0) {
		invalidRequest('siteRkey is required')
	}
...
export const xrpcRoutes = () => {
	...
	addQueryWithAliases(
		...
		['place.wisp.v2.domain.getstatus', 'place.wisp.v2.domain.get-status'],
		{
			async handler({ params, request }) {
				...
				if (auth && ownedByCaller === false && (kind === 'wisp' || (kind === 'custom' && Boolean(info.verified)))) {
					return json({
						domain,
						kind,
	...
	addProcedureWithAliases(router, withNsid(PlaceWispV2DomainClaim.mainSchema as any, XRPC_NSIDS.claim), [], {
		async handler({ input, request }) {
			const auth = requireAuthenticated(authByRequest.get(request))
			const did = auth.did as DidString

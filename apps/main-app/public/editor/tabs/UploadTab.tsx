    ...
    return (
        <div className="h-full flex flex-col border border-border/30 bg-card/50 font-mono">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-3">
                ...
                {siteMode === 'existing' ? (
                    sitesLoading ? (
                        ...
                        <div className="space-y-1">
                            <Label htmlFor="site-select" className="text-xs">Site</Label>
                            <select
                                id="site-select"
                    ...
                    <div className="space-y-1">
                        <Label htmlFor="new-site-name" className="text-xs">Site name</Label>
                        <Input
                            id="new-site-name"
                ...
                <div
                    ref={dropZoneRef}
                    className={`border-2 border-dashed p-4 flex items-center gap-3 transition-colors ${
                        isDragging
                ...
                >
                    <Upload className={`w-4 h-4 flex-shrink-0 transition-colors ${isDragging ? 'text-accent' : 'text-muted-foreground'}`} />
                    ...
                    <div className="space-y-2">
                        ...
                        {fileProgressList.length > 0 && (
                            <div className="border border-border/30 overflow-hidden">
                                ...
                                >
                                    <span>
                                        Files ({fileProgressList.filter(f => f.status === 'uploaded' || f.status === 'reused').length}/{fileProgressList.length})
                                    </span>
                                ...
                                {showFileProgress && (
                                    <div className="max-h-48 overflow-y-auto p-2 space-y-0.5 bg-background">
                                        {fileProgressList.map((file, idx) => (
                                            <div key={idx} className="flex items-start gap-2 text-xs p-1.5 hover:bg-muted/30">
                                                {file.status === 'checking' && <Loader2 className="w-3 h-3 mt-0.5 animate-spin text-blue-500 shrink-0" />}
                                                {file.status === 'uploading' && <Loader2 className="w-3 h-3 mt-0.5 animate-spin text-purple-500 shrink-0" />}
                                                {file.status === 'uploaded' && <CheckCircle2 className="w-3 h-3 mt-0.5 text-green-500 shrink-0" />}
                                ...
                                <div className="ml-5 space-y-1 max-h-32 overflow-y-auto">
                                    {failedFiles.slice(0, 10).map((file, idx) => (
                                        <div key={idx}>
                                            ...
                                            <span className="text-muted-foreground"> — {file.error}{file.size > 0 && ` (${(file.size / 1024).toFixed(1)}KB)`}</span>
                                        </div>
                                    ))}
                                    {failedFiles.length > 10 && <div className="text-muted-foreground">…and {failedFiles.length - 10} more</div>}
                                </div>
                            </div>
                        )}
                        ...
                        {skippedFiles.length > 0 && (
                            <div className="p-3 bg-yellow-500/10 border border-yellow-500/20 text-xs space-y-1">
                                ...
                                <div className="ml-5 space-y-0.5 max-h-24 overflow-y-auto">
                                    ...
                                    {skippedFiles.length > 5 && <div className="text-muted-foreground">…and {skippedFiles.length - 5} more</div>}
                                </div>
                            </div>
                        )}
                    </div>

...
function renderDomainList(data: DomainListOutput): void {
  ...
  for (const domain of domains) {
    ...
    console.log(
      `- ${pc.bold(domain.domain)} [${domain.kind}] ${statusColor(domain.status)}${mappedSite}`,
    );
  }
}
...
function renderSiteList(data: SiteListOutput): void {
  ...
  for (const site of sites) {
    const title = site.displayName
      ? `${site.siteRkey} (${site.displayName})`
      : site.siteRkey;
    ...
    for (const domain of site.domains) {
      const statusColor = domain.status === 'verified' ? pc.green : pc.yellow;
      console.log(
        `  ${domain.domain} [${domain.kind}] ${statusColor(domain.status)}`,
      );
    }
  }
}
...
export async function listDomains(
  ...
  const { agent, serviceDid } = await authenticateForXrpc(
    identifier,
